...
const [selectedOrderId, setSelectedOrderId] = useState(null);
  ...
  useEffect(() => {
    if (user?._id) {
      axios.get(`http://localhost:5005/api/orders/${user._id}`)
...
const confirmCancel = async () => {
  try {
    const res = await axios.put(`http://localhost:5005/api/orders/${selectedOrderId}/cancel`);
    setOrders((prev) =>
      prev.map((order) =>
...
const canCancel = (createdAt) => {
  ...
  const diffMinutes = (now - createdTime) / 60000; // milliseconds to minutes
  return diffMinutes <= 10;
};

  return (
    <div className="p-6 max-w-4xl mx-auto mt-10">
      <h1 className="text-2xl font-bold mb-4 text-[#FF4C29]">My Orders</h1>
