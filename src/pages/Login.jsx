...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5005/api/auth/signin", formData, {
        withCredentials: true, // For cookie-based login
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FFF5EB] dark:bg-gray-900 transition-colors duration-300 px-4">
      <div className="flex w-full max-w-5xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
        ...
        <img
          ...
          <div className="absolute top-0 right-0 h-full">
            <svg
              ...
              className="h-full w-8 fill-[#FFF5EB] dark:fill-gray-800"
            >
        ...
        {/* Form Section */}
        <div className="w-full md:w-1/2 p-8">
          <h2 className="text-3xl font-bold text-[#FF4C29] dark:text-yellow-400 mb-4 text-center">
            Welcome Back
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <input
                ...
                value={formData.email}
                onChange={handleChange}
                className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#FF4C29]"
                required
              />
            </div>
            <div>
          ...
          <p className="mt-4 text-center text-sm text-gray-600 dark:text-gray-400">
            Don’t have an account?{" "}
            <a href="/register" className="text-[#FF4C29] font-medium">
              Register
            </a>
          </p>
        </div>
